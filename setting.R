...
theme_line <- function() {
  theme_minimal(base_size = 12, base_family = base_family) %+replace%
    theme(
      ...
      plot.title = element_text(size = 20, color = cols[8], family = bold_family, margin = margin(5, 5, 5, 5)),
      # plot.subtitle = element_text(size = 15, hjust = -0.05, color = cols[8],
      #                              margin = margin(5, 5, 5, 5)),
      plot.background = element_rect(fill = "white", color = "white"),
      plot.title.position = "plot",
      legend.position = "right",
      legend.title = element_text(color = cols[8]),
...
save_plot <- function(plot, fname, width = 10, height = 5) {
  ggsave(plot = plot, filename = fname, width = width, height = height, dpi = 272)
}
